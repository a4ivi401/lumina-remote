...
echo "### Waiting for Nginx to start up and read dummy certificates..."
sleep 5
echo
...
if [ -z "$email" ]; then
    echo "Warning: no email configured."
else
    email_arg="--email $email --no-eff-email"
fi

enable_staging=""
if [ $staging != "0" ]; then enable_staging="--staging"; fi
