...
* **Движок:** `Tauri` (Rust + React/Vue/Svelte). 
* *Обоснование:* Минимальный вес (без встроенного Chromium, используется системный WebView), идеальная интеграция с Rust-бэкендом, простота создания красивого UI и легкий порт на мобильные платформы (iOS/Android) в будущем.

### Сетевое обнаружение (LAN)
* **Протокол:** mDNS / DNS-SD. Библиотека: `mdns` или `tokio-mdns`.
